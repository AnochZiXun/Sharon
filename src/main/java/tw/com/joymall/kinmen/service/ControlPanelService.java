package tw.com.joymall.kinmen.service;
...
import tw.com.joymall.kinmen.entity.Shelf;
import tw.com.joymall.kinmen.entity.Staff;
import tw.com.joymall.kinmen.repository.BannerRepository;
import tw.com.joymall.kinmen.repository.BulletinRepository;
import tw.com.joymall.kinmen.repository.FrequentlyAskedQuestionRepository;
import tw.com.joymall.kinmen.repository.InternalFrameRepository;
...
import tw.com.joymall.kinmen.repository.ShelfRepository;
...
public class ControlPanelService {
	...
	public void loadMerchandise(Merchandise entity, Element parentNode) {
		...
		Boolean recommended = entity.isRecommended();//推薦
		Utils.createElementWithTextContent("recommended", parentNode, recommended == null ? "false" : Boolean.toString(recommended));

		Integer itemWeight = entity.getItemWeight();//商品毛重(g)
		Utils.createElementWithTextContent("itemWeight", parentNode, itemWeight == null ? "" : itemWeight.toString());

		String itemModel = entity.getItemModel();//商品規格
		Utils.createElementWithTextContent("itemModel", parentNode, itemModel == null ? "" : itemModel);

		String itemUnit = entity.getItemUnit();//商品單位
		Utils.createElementWithTextContent("itemUnit", parentNode, itemUnit == null ? "" : itemUnit);

		String itemSId = entity.getItemSId();//商品報備編號
		Utils.createElementWithTextContent("itemSId", parentNode, itemSId == null ? "" : itemSId);
	}
	 ...
	 * @return 錯誤訊息
	 */
	public String saveMerchandise(Merchandise entity, Shelf shelf, String name, int price, String html, Boolean carrying, Boolean recommended, boolean withItemSId, Integer itemWeight, String itemModel, String itemUnit, String itemSId, Element parentNode) {
		String errorMessage = null;
		...
		if (withItemSId) {
			if (itemSId == null) {
				errorMessage = "商品報備編號必填！";
			}
		} else {
			if (itemWeight == null || itemModel == null || itemUnit == null) {
				errorMessage = "「商品毛重(g)、商品規格、商品單位」皆為必填！";
			}
			if (itemWeight <= 0) {
				errorMessage = "「商品毛重(g)」須大於零公克！";
			}
		}
		...
		if (errorMessage == null) {
			...
			if (withItemSId) {
				...
				entity.setItemWeight(itemWeight);
				entity.setItemModel(itemModel);
