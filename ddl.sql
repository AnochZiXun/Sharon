...
COMMENT ON COLUMN"Staff"."merchantId"IS'iCarry特店代號';
COMMENT ON COLUMN"Staff"."merchantKey"IS'iCarry商家加密金鑰';
...
COMMENT ON COLUMN"Merchandise"."itemSId"IS'商品報備編號';
COMMENT ON COLUMN"Merchandise"."itemWeight"IS'商品毛重(g)';
COMMENT ON COLUMN"Merchandise"."itemModel"IS'商品規格';
COMMENT ON COLUMN"Merchandise"."itemUnit"IS'商品單位';
...
INSERT INTO"PacketStatus"("id","name")VALUES('-1',E'被取消');
...
COMMENT ON COLUMN"Packet"."packetStatus"IS'訂單狀態';
COMMENT ON COLUMN"Packet"."orderNo"IS'特店訂單編號';
COMMENT ON COLUMN"Packet"."timestamp"IS'時間戳';
COMMENT ON COLUMN"Packet"."checkCode"IS'資料檢查碼';
...
CREATE SEQUENCE"ConceptTechHistory_id_seq"MAXVALUE 9223372036854775807 CYCLE;
CREATE TABLE"ConceptTechHistory"(
	"id"int8 DEFAULT"nextval"('"ConceptTechHistory_id_seq"'::"regclass")PRIMARY KEY,
...
ALTER SEQUENCE"ConceptTechHistory_id_seq"OWNED BY"ConceptTechHistory"."id";
COMMENT ON COLUMN"ConceptTechHistory"."id"IS'主鍵^64';
COMMENT ON COLUMN"ConceptTechHistory"."packet"IS'訂單';
COMMENT ON COLUMN"ConceptTechHistory"."accessToken"IS'一次性交付密碼';
COMMENT ON COLUMN"ConceptTechHistory"."transactionNo"IS'交易編號';
COMMENT ON COLUMN"ConceptTechHistory"."timestamp"IS'訂單建立時間戳';
COMMENT ON COLUMN"ConceptTechHistory"."rtnCode"IS'回傳代碼';
COMMENT ON COLUMN"ConceptTechHistory"."rtnMsg"IS'回傳訊息';
COMMENT ON COLUMN"ConceptTechHistory"."checkCode"IS'資料檢查碼';
COMMENT ON TABLE"ConceptTechHistory"IS'iCarry支付歷程';
